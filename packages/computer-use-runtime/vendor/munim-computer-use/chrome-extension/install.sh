...
EXTENSION_ID="limncohahkklkbhmhohkhdfdbakpnkei"
HOST_NAME="com.pine.computer_use"

here=$(cd "$(dirname "$0")" && pwd)
# macOS builds the Swift package; Linux builds the Rust crate that also covers
# Windows. Pine's packaged binary is called pine-computer-use.
case "$(uname -s)" in
  Darwin)
    for candidate in \
      "$here/../bin/pine-computer-use" \
    ...
    ;;
  *)      default_binary="$here/../bin/pine-computer-use" ;;
esac
...
case "$(uname -s)" in
  Darwin) support="$HOME/Library/Application Support/Pine/computer-use" ;;
  *)      support="${XDG_DATA_HOME:-$HOME/.local/share}/pine/computer-use" ;;
esac
mkdir -p "$support"
...
for profile in "$@"; do
  ...
  mkdir -p "$dir"
  for host_name in "$HOST_NAME"; do
    cat > "$dir/$host_name.json" <<EOF
{
  "name": "$host_name",
  "description": "Pine Computer Use browser bridge",
  "path": "$wrapper",
  "type": "stdio",
