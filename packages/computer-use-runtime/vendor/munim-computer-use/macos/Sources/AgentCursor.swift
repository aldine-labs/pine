...
// `PineAgentCursor.app`. Preferred launch is `NSWorkspace` (registers with
...
private let overlayAppName = "PineAgentCursor.app"
private let overlayExecutableName = "PineAgentCursor"
private let overlayBundleIdentifier = "com.pine.computer-use.agent-cursor"
...
final class AgentCursor {
    ...
    private func ensureRunning() {
        ...
        guard let appURL = OverlayBundle.ensureApp() else {
            fputs("pine-computer-use: agent cursor: could not materialise PineAgentCursor.app\n", stderr)
            pending.removeAll()
            return
        }
...
private enum OverlayBundle {
    static func ensureApp() -> URL? {
        ...
        // Staged artifact: `…/pine-computer-use/PineAgentCursor.app` beside the binary.
    ...
    private static func overlayInfoPlist() -> String {
        ...
        <dict>
        	...
        	<string>Pine Agent Cursor</string>
